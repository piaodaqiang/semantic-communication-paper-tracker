...
def run_daily_inbox(root: Path, max_results: int | None = None) -> dict[str, Path | int]:
    ...
    base_url = arxiv_config.get("base_url", "https://export.arxiv.org/api/query")
    default_max = int(arxiv_config.get("max_results_per_keyword", 50))
    timeout = int(arxiv_config.get("request_timeout_seconds", 30))
    max_results = max_results or default_max
    collected: list[Paper] = []
    errors: list[dict[str, str]] = []

    for keyword in keywords_config.get("primary_keywords", []):
        try:
            collected.extend(fetch_arxiv(keyword, base_url=base_url, max_results=max_results, timeout=timeout))
        except Exception as exc:
            errors.append({"keyword": keyword, "error": repr(exc)})

    recent = filter_recent(collected, years=6)
